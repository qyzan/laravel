          ...
          <main id="main">
            ...
            <section class="breadcrumbs">
              <div class="container">
                ...
                <form class="row g-4 needs-validation" novalidate action="{{ route('upload.store') }}" method="POST" enctype="multipart/form-data">
                  @csrf
                  <div class="col-md-6">
                    <label for="nama" class="form-label">Name</label>
                    <input type="text" name='nama' value="{{ old('nama') }}" class="form-control @error('nama') border border-danger @enderror" aria-describedby="nama" placeholder="Name">
                    <div class="text-xs text-red-600">@error('nama') {{ $message }} @enderror</div>
                  </div>
                  <div class="col-md-6">
                    <label for="tag" class="form-label">Tag</label>
                    <input type="text" name="tag" value="{{ old('tag') }}" class="form-control @error('tag') border border-danger @enderror" id="tag" placeholder="Tag" required>
                    <div class="text-xs text-red-600">@error('tag') {{ $message }} @enderror</div>
                  </div>
                  <div class="mb-3">
                    <label for="deskripsi" class="form-label">Description</label>
                    <textarea name="deskripsi" class="form-control @error('deskripsi') border border-danger @enderror" id="deskripsi" rows="3" placeholder="Your Document Description....">{{ old('deskripsi') }}</textarea>
